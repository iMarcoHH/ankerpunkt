...
export default function EinnahmenPage() {
  ...
  const [form, setForm] = useState({ amount: '', category: 'Gehalt', description: '', date: new Date().toISOString().split('T')[0], is_recurring: false, recurrence: 'monthly' })

  async function load() {
    const sb = createClient()
    const { data: { user } } = await sb.auth.getUser()
    if (!user) return
    const { data } = await sb.from('transactions').select('*').eq('user_id', user.id).eq('type', 'income').order('date', { ascending: false })
    setItems(data || [])
    setLoading(false)
  }
  ...
  async function save() {
    ...
    await sb.from('transactions').insert({ user_id: user.id, type: 'income', amount: parseFloat(form.amount), category: form.category, description: form.description, date: form.date, is_recurring: form.is_recurring, recurrence: form.recurrence })
    setShow(false)
    setForm({ amount: '', category: 'Gehalt', description: '', date: new Date().toISOString().split('T')[0], is_recurring: false, recurrence: 'monthly' })
    load()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="text-xs font-mono-ak text-[#C8392B] tracking-widest uppercase mb-1">// Eingang</div>
          <h1 className="font-bebas text-4xl tracking-wider text-[#0D1B2A]">EINNAHMEN</h1>
      ...
      {loading ? <div className="text-[#9AA0A6]">Lädt...</div> : (
        <div className="flex flex-col gap-3">
          {items.length === 0 && <div className="bg-white rounded-lg p-8 text-center text-[#9AA0A6]">Noch keine Einnahmen eingetragen.</div>}
          {items.map(item => (
            <div key={item.id} className="bg-white rounded-lg p-4 flex items-center justify-between border border-[#E8DFD0] border-l-4 border-l-[#C8392B]">
              <div>
      ...
      {show && (
        <div className="modal-overlay" onClick={() => setShow(false)}>
          <div className="modal-box p-6" onClick={e => e.stopPropagation()}>
            <h2 className="font-bebas text-2xl tracking-wider mb-4">EINNAHME HINZUFÜGEN</h2>
            <div className="flex flex-col gap-3">
              ...
              <select className="ak-input" value={form.category} onChange={e => setForm({...form, category: e.target.value})}>
                {['Gehalt','Freelance','Investments','Mieteinnahmen','Sonstiges'].map(c => <option key={c}>{c}</option>)}
              </select>
              ...
              <div className="flex gap-3 mt-2">
                <button onClick={() => setShow(false)} className="flex-1 py-2 border border-[#E8DFD0] rounded-lg text-[#9AA0A6] hover:bg-[#F4F2EE]">Abbrechen</button>
                <button onClick={save} className="flex-1 py-2 bg-[#C8392B] text-white rounded-lg font-medium hover:bg-[#a82e22]">Speichern</button>
              </div>
            </div>
          </div>
        </div>
